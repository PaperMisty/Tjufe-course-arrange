...
with open(path,'r',encoding='utf-8') as f:
  ...
  for content in contents[1:]:
    ...
    # 默认只上两节（一单）课
    schedule = schedule[2:]
    sub_dict['start_course'] = schedule.split('-')[0]
    if schedule[-1] == "单":
      sub_dict['step'] = 2
    ...
    for week in range(struct['week_start']+struct['week_parity'],struct['week_end']+1,struct['step']):
      ...
      if struct['location'][0] in 'CDEFJM':
        ...
        if struct['start_course'] == '1':
          ...
          hours = 11
          minutes = 40
        sub_dict2['start_time'] = datetime(2023, 2, 27, time_block, 10, 0)+timedelta(days=days,minutes=minutes,hours=hours)
        ...
        if struct['start_course'] == '1':
          ...
          hours = 11
          minutes = 50
        sub_dict2['start_time'] = datetime(2023, 2, 27, time_block, 0, 0)+timedelta(days=days,minutes=minutes,hours=hours)
        sub_dict2['end_time'] = sub_dict2['start_time']+timedelta(minutes=30,hours=1)
        sub_dict2['summary'] = struct['summary']
        sub_dict2['description'] = struct['description']
        sub_dict2['location'] = struct['location']
      ...
      # 结束一周，days归位
      days = -1
  print(f'课程总数：{len(events)}')
  return events
GMT_events = arrange(8)
Beijing_events = arrange(0)
create_ics_file(GMT_events,"GMT.ics")
create_ics_file(Beijing_events,"Beijing.ics")